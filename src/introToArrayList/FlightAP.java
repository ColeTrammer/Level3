...
public class FlightAP {
	...
	private class Trip {
		...
		public int getDuration() {
			...
			return flights.get(0).departureTime.minutesUntil(flights.get(flights.size() - 1).arrivalTime);
		}
		...
		public int getShortestLayover() {
			...
			int shortestLayover = flights.get(0).arrivalTime.minutesUntil(flights.get(1).departureTime);
			for (int i = 1; i < flights.size() - 1; i++) {
				int currentLayover = flights.get(i).arrivalTime.minutesUntil(flights.get(i + 1).departureTime);
				if (currentLayover < shortestLayover) {
					shortestLayover = currentLayover;
				}
			}
