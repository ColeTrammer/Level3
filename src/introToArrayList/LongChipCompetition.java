...
public class LongChipCompetition {
	...
	public LongChipCompetition() { 
		...
		String beetleWithLongestChip = "";
		
		for (Beatle beetle : theBeatles) {
			for (Chip chip : beetle.getChips()) {
				if (chip.getLength() > longestChip) {
					longestChip = chip.getLength();
					beetleWithLongestChip = beetle.getName();
				}
			}
		}
		System.out.println(beetleWithLongestChip);
	}
